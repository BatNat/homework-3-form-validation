...
export default class Form extends Component {
    ...
    onSubmitForm = event => {
        ...
        for (let element in objErrors) {
            const input = this.state[element];
            const elem = objErrors[element];
        ...
        this.setState({ errors, valid: errorsLength === 0 })
    }

    render () {

        const {firstName, lastName, password, errors, valid} = this.state;
